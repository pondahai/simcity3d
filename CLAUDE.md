...
## 存檔系統
localStorage key `simcity3d:save`,`SAVE_VER` 版本檢查。serializeCity() 存 city(排除 Float32Array 圖層)+ 4096 格 cell 精簡陣列(br/wr 打包成 bitmask);loadCity() 還原後以 computePower()+computeStats() 重建衍生狀態。自動存檔時機:年度結算、pagehide、visibilitychange hidden、建新城市、匯入後。匯出/匯入 JSON 在新城市彈窗。
